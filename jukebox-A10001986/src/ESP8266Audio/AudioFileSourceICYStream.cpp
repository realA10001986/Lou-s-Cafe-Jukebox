...
{
    if(openNew(url)) return true;
    return false;
}

int AudioFileSourceICYStream::openNew(const char *url)
{
...
//    cb.md("Genre", false, ret.c_str());
  ...
  if (http.hasHeader(hdr[3])) {
    ...
    //Serial.printf("ICY bitrate %s\n", ret.c_str());
    if(atoi(ret.c_str()) > 128) {
        http.end();
        return ICY_ERR_BAD_BR;
    }
  }
  if (http.hasHeader(hdr[4])) {
    String ret = http.header(hdr[4]);
    //Serial.printf("Content-Type: %s\n", ret.c_str());
    if(strcmp(ret.c_str(), "audio/mpeg") && strcmp(ret.c_str(), "audio/mp3")) {
        http.end();
        Serial.printf("Content-Type: %s\n", ret.c_str());
        return ICY_ERR_BAD_CT;
    }
  }
  icyByteCount = 0;
  size = http.getSize();
