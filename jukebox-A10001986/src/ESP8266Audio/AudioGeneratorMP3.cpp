...
int AudioGeneratorMP3::DecodeNextFrame()
{
  ...
  if(frame->header.bitrate > maxBitRate) {
      #ifdef HAVE_AUDIO_LOGGER
      audioLogger->printf_P(PSTR("MP3:bitrate too high: %u (max %u)\n"), frame->header.bitrate, maxBitRate);
      #endif
      return -1;
  }
  return 1;
