...
- They are quite unsuitable for modification without major butchering. Ruining something that is a highly valued collector's item appeared inappropriate to me given the small role the prop plays in the movie. Leave it to the Jukebox crowd.
...
For information on installing or updating the firmware of your Jukebox, see [here](#firmware-installation--firmware-update).
...
Limitations: TLS/SSL not supported; ".local" domains (MDNS, Bonjour) not supported; the machine running the broker must respond to PING (ICMP) echo requests. MQTT is disabled when your Jukebox is operated in AP-mode or when connected to the TCD run in AP-Mode (TCD-AP).
...
- __First track__. The transmitted _value_ must be a number which is either 0 or 1. If the remote player does not send this over the backchannel, it can be substituted by entering ```=0``` or ```=1```. Not required for Jukebox numbering.
- __Last track__. The transmitted _value_ must be a number (>= first track) or a Jukebox numbering term (eg. ```K-10```). If the remote player does not send this over the backchannel, it can be substituted by entering __=*value*__, _value_ being the highest Current Track can be (eg. ```=99``` or ```=K-10```).
...
The entered _values_ for State and Shuffle mode are interpreted as strings. Leave the _value_ fields empty if the information comes as boolean. (Writing "true" into the _value_ field will not work if the transmitted data isn't a string.)
...
<table>
    ...
    <tr>
     <td align="left"><img src="img/s_mp.png" title="1 flash of 300ms"></td>
     <td align="left">Music Player mode. Shown on startup and on mode changes.</td>
    </tr>
    <tr>
     <td align="left"><img src="img/s_st.png" title="2 flashes of 300ms"></td>
     <td align="left">Streaming mode. Shown on startup and on mode changes.</td>
    </tr>
    <tr>
     <td align="left"><img src="img/s_rem.png" title="3 flashes of 300ms"></td>
     <td align="left">Remote mode. Shown on startup and on mode changes.</td>
    </tr>
    <tr>
     <td align="left"><img src="img/s_alm.png" title="4 blinks, 1 second each"></td>
    ...
    <tr>
     <td align="left"><img src="img/s_err.png" title="2 brief blinks, 1 second pause, repeat"></td>
     <td align="left">Error: Sound pack installation error</td>
    </tr>
    <tr>
     <td align="left"><img src="img/s_ok.png" title="1 brief blink"></td>
     <td align="left">OK</td>
    </tr>
    <tr>
     <td align="left"><img src="img/s_nomus.png" title="1 second lit"></td>
     <td align="left">No music available to play; no stream; no music on remote player</td>
    </tr>
    <tr>
     <td align="left"><img src="img/s_shoff.png" title="1 second lit, 1 short blink"></td>
     <td align="left">Shuffle mode is off. Shown upon toggling shuffle mode</td>
    </tr>
    <tr>
     <td align="left"><img src="img/s_shon.png" title="1 second lit, 2 short blinks"></td>
     <td align="left">Shuffle mode is on. Shown upon toggling shuffle mode</td>
    </tr>
    <tr>
     <td align="left"><img src="img/s_ipdel.png" title="Continuous very brief blinks"></td>
